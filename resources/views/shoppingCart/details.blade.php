...
<section class="container-fluid my-5 dashboard">
        <div class="row m-0 p-0">
            <div class="col-12">
                <a class="btn back mb-5"  @if($shoppingCart->status_cart_id == 1) href={{route('shoppingCart.index_orders')}} @else href={{route('shoppingCart.index')}} @endif>{{__('translation.products.create.back')}}</a>
                    ...
                    @foreach($shoppingCart->products as $key => $product)
                        <tr>
                            ...
                            <td class="align-middle">
                                <a class="btn btn-warning text-black" href="{{route('shoppingCart.edit_product', ['q' => $product->pivot->quantity, 'shoppingCart' =>$shoppingCart ,'product' => $product])}}">{{__('translation.products.index.edit')}}</a>
                            </td>
                            <td class="align-middle">
